...
void ServerStart() {
	char* config = "C:\\Develop\\GamingAnyWhere\\ga\\vs2010\\Debug\\config\\server.desktop.conf";

	int notRunning = 0;
#ifdef WIN32
	if(CoInitializeEx(NULL, COINIT_MULTITHREADED) < 0) { // 쓰레드 초기화
		fprintf(stderr, "cannot initialize COM.\n");
		return;
	}
#endif

	if(ga_init(config, NULL) < 0)	{ return; }
	ga_openlog();

	if(rtspconf_parse(rtspconf_global()) < 0) { return; }
	...
	if(load_modules() < 0)	 	{ return; }
	if(init_modules() < 0)	 	{ return; }
	if(run_modules() < 0)	 	{ return; }
