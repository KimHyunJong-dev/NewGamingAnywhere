...
sdlmsg_replay_native(sdlmsg_t *msg) {
	INPUT in;
	sdlmsg_keyboard_t *msgk = (sdlmsg_keyboard_t*) msg;
	sdlmsg_hook_keyboard_t *msgh = (sdlmsg_hook_keyboard_t*)msg;
	sdlmsg_mouse_t *msgm = (sdlmsg_mouse_t*) msg;
	//
	switch(msg->msgtype) {
	case SDL_EVENT_MSGTYPE_HOOK_KEYBOARD:
		bzero(&in, sizeof(in));
		in.type = INPUT_KEYBOARD;
		in.ki.wVk = msgh->keyCode;
		in.ki.wScan = msgh->keyCode;
		if (msgh->is_pressed == 0) {
			in.ki.dwFlags |= KEYEVENTF_KEYUP;
		}
		SendInput(1, &in, sizeof(in));
		//MessageBoxA(NULL, "asf", "notice", MB_OK);
		break;
	case SDL_EVENT_MSGTYPE_KEYBOARD:
		bzero(&in, sizeof(in));
		in.type = INPUT_KEYBOARD;
