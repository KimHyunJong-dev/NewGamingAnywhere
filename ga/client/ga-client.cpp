...
#include <pthread.h>
#include <SDL2/SDL.h>
#include <SDL2/SDL_syswm.h> // Keyboard Hook 관련
#ifndef ANDROID
...
/*
	키보드 메시지를 전달받아 처리하는 함수입니다.
*/
LRESULT CALLBACK keyProc(int  code, WPARAM wParam, LPARAM lParam) {
	if (code >= 0/* && wParam == 0x100*/) {// 0x100 : 키다운
		HWND currentWindow = GetForegroundWindow();

		if (currentWindow == sdlHandle) {
			PKBDLLHOOKSTRUCT key = (PKBDLLHOOKSTRUCT)lParam;

			sdlmsg_t m;

			sdlmsg_hook_keyboard(&m, wParam == 0x100 ? 1 : 0, key->vkCode);
			ctrl_client_sendmsg(&m, sizeof(sdlmsg_hook_keyboard_t));

#ifdef DEBUG
			char keyMsg[100];
			snprintf(keyMsg, sizeof(keyMsg), "sdl: %d, current: %d, keyCode: %d  ", sdlHandle, currentWindow, key->vkCode);
			fprintf(stderr, keyMsg);
			// SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "info", keyMsg, mainWindow);
#endif
			return 0;
		}
...
create_overlay(struct RTSPThreadParam *rtspParam, int ch) {
	...
	if (SDL_GetWindowWMInfo(surface, &sysInfo)) {
		mainWindow = surface;
		sdlHandle = sysInfo.info.win.window;

		// HOOK 타입 : 2 (WM_KEYBOARD), 13 (WM_KEYBOARD_LL)
		keyHook = SetWindowsHookEx(13, keyProc, 0, 0); // threadid 0 : 전역

		if (keyHook == 0) {
			rtsperror("keyboard hooking error.");
			exit(-1);
		}
	}
	//
	...
	int       nCmdShow)
{
	...
	//
#ifdef _DEBUG
	__argv[1] = "D:\\개발\\GA\\ga\\vs2010\\Debug\\config\\client.abs.conf";
...
#else
	//
	if(ga_init(__argv[1], __argv[2]) < 0) {
		rtsperror("cannot load configuration file '%s'\n", __argv[1]);
		return -1;
	}
#endif
	...
	rtspThreadParam.url = strdup(__argv[2]);
